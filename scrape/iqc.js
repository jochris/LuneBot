...
const ASSET_DIR = join(process.cwd(), "assets", "iphone-quote");

const BG_URL = "https://raw.githubusercontent.com/ryyntwx/allimagerin/refs/heads/main/Iqcbyrin.png";
const BG_LOCAL = join(ASSET_DIR, "Iqcbyrin.png");
...
const APPLE_EMOJI_JSON_LOCAL = join(ASSET_DIR, "emoji-apple-image.json");
...
function doEnsureAssets() {
  return (async () => {
    await mkdir(ASSET_DIR, { recursive: true });
    for (const f of INTER_FONTS) {
      const dest = join(ASSET_DIR, f.file);
      if (!existsSync(dest)) {
...
function ensureAssets() {
  if (!assetsPromise) {
    assetsPromise = doEnsureAssets().catch((err) => {
      assetsPromise = null;
      throw err;
    });
  }
...
export async function renderIphoneQuote(text, time) {
  await ensureAssets();
  const s = { ...DEFAULTS, text, time };
  ...
  if (s.showReaction) {
    ...
    const rxWidth =
      startPad + (emojiNum - 1) * s.emojiSpacing + s.emojiSpacing * 0.5 + plusBtnW + startPad * 0.5;
    ...
    ctx.lineCap = "round";
    ctx.beginPath();
    ctx.moveTo(plusX - arm, plusY);
    ctx.lineTo(plusX + arm, plusY);
    ctx.moveTo(plusX, plusY - arm);
    ctx.lineTo(plusX, plusY + arm);
    ctx.stroke();
    ctx.restore();
  }

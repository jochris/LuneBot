...
export default {
    ...
    async execute(sock, m, args) {
        ...
        if (!target) {
            await m.reply('Silakan tag/mention, balas pesan, atau masukkan nomor anggota yang ingin dikick.');
            return;
        }
        ...
        try {
            ...
            await sock.groupParticipantsUpdate(m.from, [targetParticipant.id], 'remove');
            await m.reply('Berhasil mengeluarkan anggota tersebut.');
        } catch (err) {
            console.error('Error saat melakukan kick:', err);
            await m.reply('Gagal mengeluarkan anggota dari grup.');
        }
    }
};

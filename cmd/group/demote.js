...
export default {
    ...
    async execute(sock, m, args) {
        ...
        if (!target) {
            await m.reply('Silakan tag/mention, balas pesan, atau masukkan nomor admin yang ingin didemote.');
            return;
        }
        ...
        try {
            ...
            await sock.groupParticipantsUpdate(m.from, [targetParticipant.id], 'demote');
            await m.reply('Berhasil menurunkan jabatan admin tersebut menjadi anggota biasa.');
        } catch (err) {
            console.error('Error saat melakukan demote:', err);
            await m.reply('Gagal menurunkan jabatan admin.');
        }
    }
};

...
export default {
    ...
    async execute(sock, m, args) {
        ...
        if (!target) {
            await m.reply('Silakan tag/mention, balas pesan, atau masukkan nomor anggota yang ingin ditambahkan.');
            return;
        }
        ...
        try {
            ...
            await sock.groupParticipantsUpdate(m.from, [target], 'add');
            await m.reply('Berhasil menambahkan anggota tersebut.');
        } catch (err) {
            console.error('Error saat melakukan add:', err);
            await m.reply('Gagal menambahkan anggota ke grup. Pastikan nomor tersebut aktif dan tidak membatasi undangan grup.');
        }
    }
};
